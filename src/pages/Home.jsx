...
const Home = ({setBookId, bookId}) => {
  ...
  const wishList = async (bookId)=>{
    ...
    }).then((res)=>console.log(res)).catch((err)=>alert(err.response.data.msg))
  }
          ...
          {data? data.map((book)=>{
            ...
            return (
              <div key={_id} className='flex flex-col m-2 relative'>
                <Link onClick={()=>{setBookId(_id)}} to={`books/${_id}`}>
                  <div className=''>
                ...
                <div className='flex flex-row text-white'>
                  <button onClick={()=>wishList(_id)} className='bg-[rgb(145,63,226)] active:bg-[rgb(63,33,94)] hover:bg-[rgb(102,53,151)] mt-3 p-2'>Wish List Book</button>
                  {_id === bookId && 
                    isSpinning &&
                  ...
                  <div className='absolute right-0'>
                    <button onClick={()=>modifyBook(_id)} className='bg-[rgb(145,63,226)] active:bg-[rgb(63,33,94)] hover:bg-[rgb(102,53,151)] mt-3 p-2'>Modify Book</button>
                  </div>}
                </div>
              </div>
            )
