...
const WishList = ({setBookId, bookId}) => {
  ...
  useEffect(()=>{
    const getData = async ()=>{
      const data = await axios.request({headers: {
              ...
              return (
                <div key={_id} className='flex flex-col m-2'>
                  ...
                  <div className='text-white flex flex-row'>
                    <button onClick={()=>removeBook(_id)} className='bg-[rgb(145,63,226)] mt-3 p-2'>Remove Book</button>
                  </div>
                </div>
              )
            }):<div><i className="fa-solid fa-spinner fa-spin fa-3x"></i></div> 
          }
        </div>
      </div>
    </>
